...
const Home = () => {
  ...
  let allowedForms = userProfile?.allowedForms || []
    ...
    <div>
      <CardContainer>
        {(allowedForms.length > 0||isAdmin )&&
          formTypes
            .filter((form) => allowedForms.includes(form.label)||isAdmin)
            .map((form) => (
        ...
        {!allowedForms.length && !isLoading && !isAdmin&& (
          ...
          >
            Please reach out to an administrator to request access to Reports.
          </Message>
        )}
        {isLoading && (
          <Loader
